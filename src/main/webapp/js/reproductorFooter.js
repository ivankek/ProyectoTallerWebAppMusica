...
function setSong(event){
	...
	var path_imagen = event.target.getAttribute("src");
	var path_cancion = songInfo.parentElement.getAttribute("name");
	var nombre_cancion = songInfo.children.item(1).innerText;
	var artista = songInfo.children.item(2).innerText;
	
	audio.src = path_cancion;
	songName.innerText = nombre_cancion;
	artistName.innerText = artista;
	imgReproductor.setAttribute("src", path_imagen);
	playpause();
}
