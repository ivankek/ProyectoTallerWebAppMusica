...
public class CancionGeneroTest extends SpringTest{
	...
	public void conseguirGenerosDeUnaCancion() {
		...
		List<CancionGenero>generosDelTema1 = sessionFactory.getCurrentSession().createCriteria(CancionGenero.class)
				                                                               .add(Restrictions.eq("cancion", tema1))
				                                                               .list();
		
		List<CancionGenero>generosDelTema2 = sessionFactory.getCurrentSession().createCriteria(CancionGenero.class)
				                                                               .add(Restrictions.eq("cancion", tema2))
				                                                               .list();
		
		assertEquals(2 , generosDelTema1.size());
		assertEquals(1 , generosDelTema2.size());
	...
	public void conseguirGenerosDeUnAlbum() {
		...
		assertEquals(3 , generosDelAlbum.size());
	}
	...
	public void testVerQueGenerosEscuchaElUsuarioPorFavoritos() {
		...
		Favorito registro_2 = new Favorito();
		...
		registro_2.setCancion(sessionFactory.getCurrentSession().get(Cancion.class, cancionID2));
        registro_2.setUsuario(sessionFactory.getCurrentSession().get(Usuario.class , userID));
        
		sessionFactory.getCurrentSession().save(registro_1);
		sessionFactory.getCurrentSession().save(registro_2);
		...
		List<CancionGenero> generosEncontrados;
		
		for(Favorito item : listaDeFavoritos) {
			 generosEncontrados = sessionFactory.getCurrentSession().createCriteria(CancionGenero.class)
					                                                .add(Restrictions.eq("cancion" , item.getCancion())) 
					                                                .list();
			 for(CancionGenero itemCG : generosEncontrados) {
				 if(!generosDelUsuario.contains(itemCG.getGenero().getNombre())) {
					 generosDelUsuario.add(itemCG.getGenero().getNombre());
				 }
			 }
		}
		
		assertEquals(3 , generosDelUsuario.size());
	}
}
